...
class ConfiguredPory(Pory, t.Generic[t_ConfigType]):
    ...
    def __post_init__(self, app_id: str, config_model: type[t_ConfigType]):
        ...
        for guild_id in data:
            self.guild_configs[guild_id] = disc.cast(config_model, data[guild_id])
        ...
        for field in dc.fields(config_model):
            field_type = field.type if not t.get_origin(field.type) == list else t.get_args(field.type)
            if not field_type is api.Snowflake: continue
            ...
            if t.get_origin(field.type) == list:
                ...
                def add(channel: api.Channel, *, guild_id: api.Snowflake | None):
                    ...
                    channel_list: list[api.Snowflake] = getattr(self.guild_configs[guild_id], field.name)
                    channel_list.append(channel.id)
                    return api.ResponseMessage(
                        content=f"Channel {channel.name} added to {field.name} channels."
                    )
                @setters
                def remove(channel: api.Channel, *, guild_id: api.Snowflake | None):
                    ...
                    return api.ResponseMessage(content=f"Channel {channel.name} removed from {field.name} channels.")
            else:
                @setters
                def set(channel: api.Channel, *, guild_id: api.Snowflake | None):
                    if not guild_id:
                        return api.ResponseMessage(content="You can't use this command outside of a guild.")
                    setattr(self.guild_configs[guild_id], field.name, channel.id)
                    return api.ResponseMessage(content=f"Channel {channel.name} has been set as the {field.name} channel.")
